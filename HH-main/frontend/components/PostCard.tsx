    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg hover:-translate-y-0.5 transition-all duration-300 ease-out flex flex-col h-full relative group/card gpu-accelerated z-10">
      ...
      <div className="p-6 flex-1">
        ...
        <div className={`prose prose-sm prose-slate max-w-none text-gray-600 relative ${expanded ? '' : 'max-h-40 overflow-hidden'}`}>
             {(() => {
               // 选择要显示的内容：优先使用 processedContent，如果为空则使用 originalContent
               const processed = (post.processedContent || '').trim();
               const original = (post.originalContent || '').trim();
               
               // 如果没有内容，不显示
               if (processed.length <= 50 && original.length <= 50) {
                 return null;
               }
               
               // 优先使用 processedContent，如果为空或太短则使用 originalContent
               const useProcessed = processed.length > 50;
                 ...
                 <ReactMarkdown components={{
                     ...
                     <div className="text-gray-700 leading-relaxed">
                       {(() => {
                         ...
                         const preview = expanded ? textContent : textContent.substring(0, 500);
                         return <p className="whitespace-pre-wrap">{preview}{!expanded && textContent.length > 500 ? '...' : ''}</p>;
                       })()}
                     </div>
                   )}
