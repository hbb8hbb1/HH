...
import { 
  ...
  User, Bookmark, CornerDownRight, Copy, Crown, ShieldCheck, UserX
} from 'lucide-react';
      ...
      {isReplying && (
        <form onSubmit={handleReplySubmit} className={`mt-3 mb-2 flex gap-2 animate-in slide-in-from-top-1 duration-200 ${isRoot ? 'pl-11' : 'pl-9'}`}>
          <div className="flex-1 relative">
             <input autoFocus type="text" placeholder={`回复 @${comment.author}...`} className="w-full px-4 py-2 text-xs bg-white border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-300 transition-all shadow-sm" value={replyText} onChange={(e) => setReplyText(e.target.value)} />
          </div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-200 flex flex-col h-full relative group/card">
      ...
      <div className="absolute top-4 right-4 z-10 flex gap-2">
         {post.isAnonymous && (
           <div className="bg-gray-100 text-gray-500 text-[9px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1 border border-gray-200 shadow-sm">
             <UserX size={10} /> 匿名
           </div>
         )}
         {post.authorIsPro && (
      ...
      <div className="p-6 flex-1">
        <div className="flex justify-between items-start mb-4">
          <div className="flex-1 min-w-0 pr-12">
            <div className="flex items-center gap-2 text-sm text-blue-600 font-semibold mb-1">
              <Building size={14} /> <span>{post.company}</span>
              <span className="text-gray-300">|</span>
              <Briefcase size={14} /> <span>{post.role}</span>
            </div>
            <h3 className="text-xl font-bold text-gray-800 leading-tight group-hover/card:text-blue-600 transition-colors">{post.title}</h3>
          </div>
          <div className="flex flex-col items-end pt-1">
             <div className="flex items-center gap-1 mb-1">{renderStars(post.difficulty)}</div>
             ...
             <span className={`text-[10px] mt-1 font-medium flex items-center gap-1 ${post.authorIsPro ? 'text-amber-600' : 'text-gray-400'}`}>
                {post.isAnonymous ? <UserX size={10} /> : (post.authorIsPro ? <Crown size={10} className="fill-amber-500" /> : <User size={10} />)}
                @{post.isAnonymous ? '某位面试官' : (post.authorName || '匿名')}
             </span>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
            {post.tags.map((tag, idx) => (
                <span key={idx} className="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-bold bg-gray-50 text-gray-400 border border-gray-100 group-hover/card:border-blue-100 group-hover/card:text-blue-400 transition-all uppercase">
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between border-t border-gray-50 pt-4 gap-2">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button onClick={handleShare} className={`flex items-center text-sm font-bold transition-colors ${copied ? 'text-green-600' : 'text-gray-500 hover:text-gray-700'}`}>
                   {copied ? <Check size={16} className="mr-1.5" /> : <Share2 size={16} className="mr-1.5" />}
                   {copied ? '已复制' : '分享'}
                </button>
                <button onClick={() => onToggleFavorite?.(post.id)} className={`flex items-center text-sm font-bold transition-colors ${post.isFavorited ? 'text-indigo-600' : 'text-gray-500 hover:text-gray-700'}`}>
        ...
        <div className="bg-slate-50/80 border-t border-gray-100 animate-in slide-in-from-top-2 duration-200">
          <div className="p-5 space-y-6 max-h-[500px] overflow-y-auto custom-scrollbar">
            {post.comments.length > 0 ? post.comments.map(comment => (
              <CommentItem key={comment.id} comment={comment} postId={post.id} onReply={(pid, content, parentId) => onAddComment && onAddComment(pid, content, parentId)} />
            )) : (
              <div className="flex flex-col items-center justify-center py-10 text-center text-gray-400">
                  <MessageSquare size={24} className="mb-2" />
                  <p className="text-xs font-bold uppercase tracking-widest">暂无评论</p>
              </div>
            )}
          </div>
          <div className="p-4 bg-white border-t border-gray-100">
            <form onSubmit={handleSubmitComment} className="flex gap-3 items-end">
              <div className="flex-1 relative group">
                  <textarea ref={textareaRef} placeholder="写下你的看法..." className="w-full min-h-[46px] max-h-32 py-3 px-4 bg-gray-50 border border-gray-200 focus:bg-white focus:border-blue-300 rounded-2xl text-sm focus:ring-4 focus:ring-blue-50 transition-all resize-none overflow-hidden" rows={1} value={commentText} onChange={handleInput} />
                  <button type="submit" disabled={!commentText.trim()} className="absolute right-2 bottom-2 p-2 text-white bg-blue-600 rounded-xl hover:bg-blue-700 disabled:opacity-0 transition-all">
                    <Send size={15} />
                  </button>
              </div>
            </form>
