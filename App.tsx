...
function App() {
  ...
  useEffect(() => {
    ...
    const fetchPosts = async () => {
      console.log('🔄 开始获取 posts 数据...');
      try {
        const apiUrl = '/api/posts?page=1&limit=30';
        ...
        if (response.ok) {
          ...
          // 转换后端数据格式为前端格式
          const formattedPosts: InterviewPost[] = data.map((post: any) => ({
            id: post._id || post.id,
            title: post.title || '',
          ...
          setPosts(formattedPosts.length > 0 ? formattedPosts : MOCK_POSTS);
        } else {
